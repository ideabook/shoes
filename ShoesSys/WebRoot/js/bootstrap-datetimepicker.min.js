	...
	var e = function(h, g) {
		...
		this.component = this.element.is(".date")
				? this.element
						...
						.parent()
				: false;
		this.componentReset = this.element.is(".date") ? this.element
				.find(".add-on .icon-remove").parent() : false;
		...
		this.linkField = g.linkField || this.element.data("link-field")
				|| false;
		this.linkFormat = b.parseFormat(g.linkFormat
		...
		this.picker = c(b.template).appendTo(this.isInline
				? this.element
				: "body").on({
	...
	e.prototype = {
		...
		fill : function() {
			...
			var B = new Date(this.viewDate), o = B.getUTCFullYear(), C = B
					...
					.getUTCMinutes(), t = this.startDate !== -Infinity
					? this.startDate.getUTCFullYear()
					: -Infinity, y = this.startDate !== -Infinity
					? this.startDate.getUTCMonth()
					: -Infinity, k = this.endDate !== Infinity ? this.endDate
					.getUTCFullYear() : Infinity, u = this.endDate !== Infinity
			...
			this.picker.find(".datetimepicker-days thead th:eq(1)")
					.text(d[this.language].months[C] + " " + o);
			this.picker.find(".datetimepicker-hours thead th:eq(1)").text(h
					+ " " + d[this.language].months[C] + " " + o);
			this.picker.find(".datetimepicker-minutes thead th:eq(1)").text(h
					+ " " + d[this.language].months[C] + " " + o);
			...
			this.picker.find(".datetimepicker-days tbody").empty().append(m
					.join(""));
			m = [];
			var q = "", z = "", n = "";
			for (var v = 0; v < 24; v++) {
				...
				switch (l[0].nodeName.toLowerCase()) {
					case "th" :
						switch (l[0].className) {
							...
							case "today" :
								var h = new Date();
								h = f(h.getFullYear(), h.getMonth(), h
												.getDate(), h.getHours(), h
												.getMinutes(), h.getSeconds(),
										0);
								this.viewMode = this.startViewMode;
								this.showMode(0);
								this._setDate(h);
					...
					case "span" :
						if (!l.is(".disabled")) {
							var n = this.viewDate.getUTCFullYear(), m = this.viewDate
									.getUTCMonth(), o = this.viewDate
									.getUTCDate(), p = this.viewDate
		...
		showMode : function(g) {
			...
			this.picker.find(">div").hide().filter(".datetimepicker-"
					+ b.modes[this.viewMode].clsName).css("display", "block");
			this.updateNavArrows()
		},
		reset : function(g) {
						...
						switch (g) {
							case "MM" :
								p = c(d[o].months).filter(function() {
									var i = this.slice(0, m[n].length), s = m[n]
											.slice(0, i.length);
									return i == s
								});
			...
			if (k == "standard") {
				...
				if (k == "php") {
					n = {
						...
						S : (g.getUTCDate() % 10 <= d[o].suffix.length
								? d[o].suffix[g.getUTCDate() % 10 - 1]
								: ""),
						a : (d[o].meridiem.length == 2 ? d[o].meridiem[g
								.getUTCHours() < 12 ? 0 : 1] : ""),
						g : (g.getUTCHours() % 12 == 0 ? 12 : g.getUTCHours()
								% 12),
						G : g.getUTCHours(),
						i : g.getUTCMinutes(),
						s : g.getUTCSeconds()
